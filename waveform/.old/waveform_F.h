...
wave_component(float in_phase, float in_per, float in_amp, size_t in_tS_s, size_t in_tS_e)
	: phase(in_phase), period(in_per), amp(in_amp), t_strt(in_tS_s), t_end(in_tS_s) {}

// empty ctor for first element, only pass the next update time
wave_component(size_t in_step)
	: phase(0.0), period(0.0), amp(0.0), t_strt(in_step), t_end(0) {}

};

// waveform
typedef std::vector < wave_component > waveform;
// NOTE: wf[0] is special, contains valid data only in "tStep_start" -- this tells us when the spiking behavior needs to be updated
